...
static char const rcsid[] = "$Id: gfOut.c,v 1.9 2004/05/25 20:03:49 kent Exp $";
...
/* if (score >= minMatch) Moved to higher level */
	...
	for (ff = ali; ff != NULL; ff = ff->right)
	    fprintf(f, "%d,", ff->nEnd - ff->nStart);
	fprintf(f, "\t");
	for (ff = ali; ff != NULL; ff = ff->right)
	    fprintf(f, "%d,", ff->nStart - needle);
	fprintf(f, "\t");
	for (ff = ali; ff != NULL; ff = ff->right)
...
for (sAli = ali; sAli != NULL; sAli = eAli)
    {
    eAli = ffNextBreak(sAli, 5, tSeq, t3List);
    dyStringClear(q);
    dyStringClear(t);
