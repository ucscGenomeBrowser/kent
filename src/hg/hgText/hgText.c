...
if (strcmp(position, "genome"))
    ...
    if (position != NULL && position[0] != 0)
	{
	if (!findPositionInGenome(position, &choosenChromName, &winStart,
				  &winEnd))
	    return;
	}
...
if (hFindChromStartEndFieldsDb(getTableDb(), table,
	...
	snprintf(query, 256, "SELECT * FROM %s WHERE %s = \"%s\" AND %s < %d AND %s > %d",
		 table, chromFieldName, choosenChromName, startName, winEnd,
		 endName, winStart);
	}
    }
else
...
while(current != 0)
    {
    if (strstr(current->name, "field_") == current->name &&
	strcmp(current->val, "on") == 0)
	{	
	/* make sure that the field name doesn't have anything "weird" in it */
...
void outputTabData(char* query, char* tableName, struct sqlConnection *conn,
...
char *makeFaStartLine(char *chrom, char *table, char *name, int start,
		      int size, char strand)
...
void writeOut(FILE *f, char *chrom, int start, int size, char strand,
	      DNA *dna, char *faHeader)
...
void outputDna(FILE *f, char *chrom, char *table, int start, int size,
	        char *dna, char *nibFileName, FILE *nibFile, int nibSize,
	       char strand, char *name)
...
if (!hFindBed6FieldsDb(getTableDb(), table, chromField, startField, endField,
...
if (cgiOptionalString("hgSeq.casing") != NULL)
    {
    hgSeqItemsInRangeDb(getTableDb(), table, chrom, chromStart, chromEnd,
			NULL);
    }
else
    {
...
int c;
char *hgSeq;
boolean allGenome = FALSE;	/* this flag is true if we are fetching the 
				   whole genome not just chrom by chrom */
...
/* Make sure we'll be able to see this table in the DB: */
if (allGenome)
...
if (table == NULL || existsAndEqual("phase", "table"))
	...
	{
	webStart(cart , "Table Browser on %s Freeze: Select Fields",
		 freezeName);
	getChoosenFields();
	webEnd();
	}
