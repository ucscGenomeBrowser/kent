...
static char const rcsid[] = "$Id: rqlEval.c,v 1.1 2009/12/02 01:29:57 kent Exp $";
...
static struct rqlEval rqlEvalEq(struct rqlParse *p, struct tdbRecord *ra)
...
static struct rqlEval rqlEvalLt(struct rqlParse *p, struct tdbRecord *ra)
/* Return true if r < l . */
{
struct rqlParse *lp = p->children;
struct rqlParse *rp = lp->next;
...
static struct rqlEval rqlEvalGt(struct rqlParse *p, struct tdbRecord *ra)
/* Return true if r > l . */
{
struct rqlParse *lp = p->children;
struct rqlParse *rp = lp->next;
...
static struct rqlEval rqlEvalLike(struct rqlParse *p, struct tdbRecord *ra)
...
struct rqlEval rqlEvalOnRecord(struct rqlParse *p, struct tdbRecord *ra)
/* Evaluate self on ra. */
{
struct rqlEval res;
switch (p->op)
	...
	struct tdbField *f = tdbRecordField(ra, p->val.s);
	if (f == NULL)
	    res.val.s = "";
	else
	    res.val.s = f->val;
