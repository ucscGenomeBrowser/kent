...
int main (int argc, char **argv)
{
	...
	/* reverse strand */
        for (comp = ali->components; comp; comp = comp->next)
            do_revcomp(comp->text, ali->textSize );
...
static void get_args (int argc, char **argv, int *output_mode, struct mafFile **file, char **id,
		      char **motif, int *do_order, int *nrow, float *cutoff, int *misses_allowed)
{
    int i;
...
static int motif_hit (char *text, int col, struct MOTIF *motif, int *score)
{
    int i;
