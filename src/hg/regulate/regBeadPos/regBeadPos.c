...
double *bVals = a->chromVals->valBuf;
...
for (i=0; i<letterCount; ++i)
    ...
    UBYTE letter = aQuant * ArraySize(a->cuts) + bQuant;
    letters[i] = letter;
    startWin = endWin;
    }
...
double *loEmitProb = makeEmissionProbsForLo();
double *medEmitProb = makeEmissionProbsForMed();
double *dnaseEmitProb = makeEmissionProbsForDnase();
double *histoneEmitProb = makeEmissionProbsForHistones();
dumpProb(uglyOut, loEmitProb, "lo");
dumpProb(uglyOut, medEmitProb, "med");
dumpProb(uglyOut, dnaseEmitProb, "dnase");
dumpProb(uglyOut, histoneEmitProb, "histone");
}
