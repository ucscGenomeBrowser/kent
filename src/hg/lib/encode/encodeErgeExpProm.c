/* encodeErgeExpProm.c was originally generated by the autoSql program, which also 
 * generated encodeErgeExpProm.h and encodeErgeExpProm.sql.  This module links the database and
...
#include "dystring.h"
#include "jksql.h"
#include "encode/encodeErgeExpProm.h"

static char const rcsid[] = "$Id: encodeErgeExpProm.c,v 1.1 2008/08/01 17:49:04 aamp Exp $";
...
/* Load all encodeErgeExpProm from a whitespace-separated file.
 * Dispose of this with encodeErgeExpPromFreeList(). */
{
struct encodeErgeExpProm *list = NULL, *el;
struct lineFile *lf = lineFileOpen(fileName, TRUE);
char *row[14];

while (lineFileRow(lf, row))
    {
    el = encodeErgeExpPromLoad(row);
    slAddHead(&list, el);
    }
lineFileClose(&lf);
slReverse(&list);
...
/* Load all encodeErgeExpProm from a chopper separated file.
 * Dispose of this with encodeErgeExpPromFreeList(). */
{
struct encodeErgeExpProm *list = NULL, *el;
struct lineFile *lf = lineFileOpen(fileName, TRUE);
...
{
struct encodeErgeExpProm *el, *next;

for (el = *pList; el != NULL; el = next)
    {
    next = el->next;
    encodeErgeExpPromFree(&el);
    }
*pList = NULL;
}
