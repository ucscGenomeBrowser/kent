...
hPrintf("<FORM ACTION=\"../cgi-bin/gisaidTable\" METHOD=POST ENCTYPE=\"multipart/form-data\">\n");
...
if (pasteVal == NULL || pasteVal[0] == 0)
    ...
    struct tempName tn;
    FILE *f;
    makeTempName(&tn, "gisaidTable", ".key");
...
//makeTitle("Select Subjects", "gisaidTableHelp.html#Filter");
hotLinks();
hPrintf("<FORM ACTION=\"../cgi-bin/gisaidTable\" METHOD=POST>\n");
cartSaveSession(cart);
