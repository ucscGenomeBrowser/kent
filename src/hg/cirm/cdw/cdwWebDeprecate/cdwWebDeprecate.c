/* cdwWebDeprecate - A cgi script to mark files as deprecated.. */
...
errAbort(
  "cdwWebDeprecate - A cgi script to mark files as deprecated.\n"
  "usage:\n"
  "   cdwWebDeprecate cgiVar=value cgiVar2=value2\n"
  );
}

boolean okToShowAllowBox(struct sqlConnection *conn, char *userEmail)
{
return cdwUserIsAdmin(conn,userEmail);
};
...
{
char *email = cdwFindOwnerNameFromFileId(conn, fId);
...
{
printf("Deprecate files in CIRM Data Warehouse.<BR>");
...
for (id = idList; id != NULL; id = id->next)
    {
    dyStringClear(dy);
    sqlDyStringPrintf(dy, "update cdwFile set deprecated='%s' where id=%d", reason, id->val);
...
if (isEmpty(reason))
   ...
   for (acc = accList; acc != NULL; acc = acc->next)
	...
	char query[256];
	sqlSafef(query, sizeof(query), "select fileId from cdwValidFile where licensePlate='%s'", licensePlate);
	...
	if (!okToDeprecateThisFile(conn, id, userEmail))
	    ...
	    licensePlate, cdwFindOwnerNameFromFileId(conn, id));
	    if (okToShowAllowBox(conn, userEmail))
...
{
printf("<FORM ACTION=\"../cgi-bin/cdwWebDeprecate\" METHOD=GET>\n");
...
cdwWebHeaderWithPersona("Deprecate files in CIRM Data Warehouse");
