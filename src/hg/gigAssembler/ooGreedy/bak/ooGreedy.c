...
static char const rcsid[] = "$Id: ooGreedy.c,v 1.2 2003/05/06 07:22:33 kate Exp $";

int version = 93;       /* Current version number. */
...
/* Describes a clone (BAC) */
    ...
    struct ntContig *ntContig; /* Clone is really an NT fusion if non-NULL. */
...
/* A fragment of a clone. */
    ...
    struct ntContig *ntContig;  /* If non-null fragment is really an NT contig fusion. */
...
int cmpNtClonePos(const void *va, const void *vb)
...
void checkNtClonesLoaded(struct ntContig *ntList)
/* Make sure that all clones in NT contigs are loaded. */
{
struct ntContig *nt;
...
for (nt = ntList; nt != NULL; nt = nt->next)
    ...
    for (ntPos = nt->cloneList; ntPos != NULL; ntPos = ntPos->next)
        {
	if (ntPos->clone == NULL)
	    errAbort("NT clone %s has no sequence", ntPos->name);
	++count;
	}
    }
uglyf("%d nt clones loaded ok\n", count);
}

void removeNtOrigFrags(struct ntContig *ntList, struct oogFrag **pFragList,
	struct oogFrag **pNtOrigFragList)
/* Remove frags that are part of original NT clones (not psuedo clones)
 * from frag list. */
{
struct ntContig *nt;
struct ntClonePos *ntPos;
struct oogFrag *frag, *nextFrag, *fragList = NULL, *ntOrigFragList = NULL;
struct dlNode *node;
int count = 0, remCount = 0;

/* Mark all frags that are part of original NT clones. */
for (nt = ntList; nt != NULL; nt = nt->next)
    {
    for (ntPos = nt->cloneList; ntPos != NULL; ntPos = ntPos->next)
        {
	for (node = ntPos->clone->fragList->head; !dlEnd(node); node = node->next)
	    {
	    frag = node->val;
	    frag->isNtOrigFrag = TRUE;
	    }
	}
    }

/* Move marked frags to new list. */
for (frag = *pFragList; frag != NULL; frag = nextFrag)
    {
    nextFrag = frag->next;
    if (frag->isNtOrigFrag)
        {
	slAddHead(&ntOrigFragList, frag);
	++remCount;
	}
    else
        {
...
slReverse(&ntOrigFragList);
*pNtOrigFragList = ntOrigFragList;
slReverse(&fragList);
*pFragList = fragList;
uglyf("Removed %d of %d fragments because in NTs\n", remCount, count);
}

int minMapNtPos(struct ntClonePos *posList)
/* Return smallest map position of clones in posList. */
{
struct ntClonePos *ntPos;
int minPos = BIGNUM;
assert(posList != NULL);
for (ntPos = posList; ntPos != NULL; ntPos = ntPos->next)
    {
    if (minPos > ntPos->clone->mapPos) minPos = ntPos->clone->mapPos;
    }
return minPos;
}

void addNtPsuedoClones(struct ntContig *ntList, 
    struct hash *fragHash, struct oogFrag **pFragList,
    struct hash *cloneHash, struct oogClone **pCloneList)
/* Fake up a clone with one fragment for each NT contig. */
{
struct ntContig *nt;
struct ntClonePos *pos;
...
for (nt = ntList; nt != NULL; nt = nt->next)
    {
    /* Allocate and fill in all 'early' fields. */
    AllocVar(clone);
    hel = hashAddUnique(cloneHash, nt->name, clone);
    clone->name = hel->name;
    clone->mapPos = minMapNtPos(nt->cloneList);
    clone->phase = htgFinished;
    clone->size = nt->size;
    ...
    /* Add single frag. */
    AllocVar(frag);
    hel = hashAddUnique(fragHash, nt->name, frag);
    frag->name = hel->name;
    frag->size = nt->size;
    frag->clone = clone;
    frag->seq = NULL;    /* no direct sequence here. */
    frag->ntContig = nt;
    frag->startGood = 0;
    frag->endGood = nt->size;
    frag->clonePos = 0;
    slAddHead(pFragList, frag);

    dlAddValHead(clone->fragList, frag);
    }
}
...
void ntPslTranslateOne(struct ntClonePos *pos, struct psl *psl, 
	struct hash *splitFinFragHash, boolean doQ)
/* Translate one psl from clone to NT contig coordinates. */
{
char **pName;
struct ntContig *nt = pos->ntContig;
unsigned *pSize, *pStart, *pEnd, *pStarts;
...
if (pos->size != *pSize)
    {
    /* Looks like the clone is fragmented. */
    struct splitFrag *splitFrag = hashFindVal(splitFinFragHash, *pName);
    if (splitFrag == NULL)
        warn("Size mismatch between nt and .fa file for %s", *pName);
    else
	offset += splitFrag->start;
    }
freez(pName);
*pName = cloneString(nt->name);
*pSize = nt->size;
if (pos->orientation > 0)
    ...
    *pEnd += offset;
    if (psl->strand[0] == '-' && doQ)
	{
        offset = nt->size - (pos->pos + pos->size);
	}
    ...
    {
    int cloneSize = pos->size;
    int s,e;

    s = cloneSize - *pEnd + offset;
    e = cloneSize - *pStart + offset;
    *pStart = s;
    *pEnd = e;
    psl->strand[0] = otherStrandChar(psl->strand[0]);
    if (!doQ)
	{
	for (i=0; i<blockCount; ++i)
	    {
	    psl->tStarts[i] = cloneSize - psl->tStarts[i] + offset;
	    psl->qStarts[i] = cloneSize - psl->qStarts[i] + offset;
	    }
	reverseUnsignedArray(psl->tStarts, blockCount);
	reverseUnsignedArray(psl->qStarts, blockCount);
	}
    errAbort("Negative orientation in NT contig.  Whoaa, double check code.");
    }
} 

void ntPslTranslate(struct psl *pslList, 
	struct hash *ntCloneHash, struct hash *splitFinFragHash, 
	boolean translateQ, boolean translateT)
/* Translate psl's in list that hit clones in an NT to NT
 * contig coordinates. Translate query, target, or both. */
{
struct psl *psl;
struct ntClonePos *pos;
char cloneName[128];
...
for (psl = pslList; psl != NULL; psl = psl->next)
	...
	if ((pos = hashFindVal(ntCloneHash, cloneName)) != NULL)
	    ntPslTranslateOne(pos, psl, splitFinFragHash, TRUE);
	}
    if (translateT)
        {
	fragToCloneName(psl->tName, cloneName);
	if ((pos = hashFindVal(ntCloneHash, cloneName)) != NULL)
	    ntPslTranslateOne(pos, psl, splitFinFragHash, FALSE);
	}
    }
}
...
void ntPslMergeSubClump(struct psl *clumpStart, struct psl *clumpEnd, int clumpSize, 
	boolean doQ, struct psl **pMergedList)
/* Merge one clump of Psls that have same query and target sequences,
 * are close to each other on diagonal, and where each psl overlaps. */
{
struct psl *psl, *nextPsl, *fakePsl;
struct ffAli *ffList = NULL, *ff;
struct psl acc;
int qTotal = 0;
double scale;

logIt(" merging sub clump of %d: qName %s, tName %s, diagonal %d...\n", 
	clumpSize, clumpStart->qName, clumpStart->tName,
	clumpStart->qStart - clumpStart->tStart);

/* Take care of common and easy case. */
if (clumpSize == 1)
    {
    slAddHead(pMergedList, clumpStart);
    return;
    }

/* Null terminate list. */
psl = slElementFromIx(clumpStart, clumpSize-1);
psl->next = NULL;

/* Make a ffAli struct that's a concatenation of all psls.
 * Also add up fields from all psls. */
zeroBytes(&acc, sizeof(acc));
for (psl = clumpStart; psl != NULL; psl = nextPsl)
    {
    nextPsl = psl->next;
    acc.match += psl->match;
    acc.misMatch += psl->misMatch;
    acc.repMatch += psl->repMatch;
    acc.nCount += psl->nCount;
    acc.qNumInsert += psl->qNumInsert;
    acc.qBaseInsert += psl->qBaseInsert;
    acc.tNumInsert += psl->tNumInsert;
    acc.tBaseInsert += psl->tBaseInsert;
    qTotal += psl->qEnd - psl->qStart;
    ff = pslToFakeFfAli(psl, NULL, NULL);
    ffCat(&ffList, &ff);
    logIt("   qStart %d, tStart %d\n", psl->tStart, psl->tEnd);
    }

/* Get rid of overlapping sections in ffAli space, borrowing
 * some routines from superStitcher and fuzzyFinder. */
ffAliSort(&ffList, ffCmpHitsNeedleFirst); 
ffList = ffMergeExactly(ffList, NULL, NULL);
//ffList = ffMergeNeedleAlis(ffList, TRUE);
//ffList = ffMergeHayOverlaps(ffList);
ffList = ffRemoveEmptyAlis(ffList, TRUE);

/* Convert back to psl format.  Fake match/mismatch
 * scores since don't actually have sequence.... */
fakePsl = pslFromFakeFfAli(ffList, NULL, NULL, clumpStart->strand[0], 
	clumpStart->qName, clumpStart->qSize, 
	clumpStart->tName, clumpStart->tSize);
scale = fakePsl->qEnd - fakePsl->qStart;
scale /= qTotal;
fakePsl->match = round(acc.match * scale);
fakePsl->misMatch = round(acc.misMatch * scale);
fakePsl->repMatch = round(acc.repMatch * scale);
fakePsl->nCount = round(acc.nCount * scale);
fakePsl->qNumInsert = round(acc.qNumInsert * scale);
fakePsl->qBaseInsert = round(acc.qBaseInsert * scale);
fakePsl->tNumInsert = round(acc.tNumInsert * scale);
fakePsl->tBaseInsert = round(acc.tBaseInsert * scale);

ffFreeAli(&ffList);
pslFreeList(&clumpStart);
slAddHead(pMergedList, fakePsl);
}

void ntPslMergeClump(struct psl *clumpStart, struct psl *clumpEnd, int clumpSize, 
	boolean doQ, struct psl **pMergedList)
/* Merge one clump of Psls that have same query and target sequences,
 * and are close to each other on diagonal. */
{
...
logIt("merging clump of %d: qName %s, tName %s, diagonal %d...\n", 
	clumpSize, clumpStart->qName, clumpStart->tName,
	clumpStart->qStart - clumpStart->tStart);

/* Turn clump into list sorted by relevant coordinate. */
psl = slElementFromIx(clumpStart, clumpSize-1);
psl->next = NULL;
slSort(&clumpStart, pslCmpQuery);
furthestEnd = clumpStart->qStart;
subStart = clumpStart;

for (psl = clumpStart; psl != NULL; psl = nextPsl)
    {
    nextPsl = psl->next;
    if (psl->qStart - furthestEnd > 10)
        {
	ntPslMergeSubClump(subStart, psl, subCount, doQ, pMergedList);
	subStart = psl;
	subCount = 1;
	furthestEnd = psl->qEnd;
	}
    else
        {
	++subCount;
	if (psl->qEnd > furthestEnd) furthestEnd = psl->qEnd;
	}
    }
ntPslMergeSubClump(subStart, NULL, subCount, doQ, pMergedList);
}

void ntPslMergeOverlapping(struct psl **pPslList, boolean doQ)
/* Merge Psls from different clones that now refer to 
 * same NT contig. */
{
struct psl *ntList = NULL, *otherList = NULL, *psl, *nextPsl;
struct psl *mergedNtList = NULL;
char *lastQ = "", *lastT = "";
struct psl *clumpStart = NULL;
int diag, lastDiag = 0;
int clumpSize = 0;
char lastStrand = 0;

splitPslOnPrefix(pPslList, "NT_", doQ, &ntList, &otherList);
slSort(&ntList, (doQ ? pslCmpQueryTargetDiag : pslCmpTargetQueryDiag));

for (psl = ntList; psl != NULL; psl = nextPsl)
    {
    nextPsl = psl->next;
    if (psl->strand[0] == '+')
	diag = psl->qStart - psl->tStart;
    else
        diag = (psl->qSize - psl->qEnd) - psl->tStart;
    if (!sameString(psl->qName, lastQ) || !sameString(psl->tName, lastT) ||
                   psl->strand[0] != lastStrand || intAbs(diag-lastDiag) > 10)
        {
        if (clumpStart != NULL)
            ntPslMergeClump(clumpStart, psl, clumpSize, doQ, &mergedNtList);
        clumpStart = psl;
        clumpSize = 1;
	lastQ = psl->qName;
	lastT = psl->tName;
	lastStrand = psl->strand[0];
	lastDiag = diag;
        }
    else
        {
...
if (clumpStart != NULL)
    ntPslMergeClump(clumpStart, NULL, clumpSize, doQ, &mergedNtList);
slReverse(&mergedNtList);
*pPslList = slCat(mergedNtList, otherList);
}

struct psl *loadPslAndNtFixTarget(char *fileName, 
	struct hash *ntCloneHash, struct hash *splitFinFragHash)
/* Load psl file and translate NT clones to Nt contigs. */
{
struct psl *pslList;

pslList = pslLoadAll(fileName);
ntPslTranslate(pslList, ntCloneHash, splitFinFragHash, FALSE, TRUE);
ntPslMergeOverlapping(&pslList, FALSE);
return pslList;
}

int ntContigWeightedOrientation(struct ntContig *ntContig)
/* Figure out which direction relative to map ntContig wants to go. */
{
int diffTotal = 0;
int diffOne;
struct ntClonePos *ntPos;
struct oogClone *clone, *lastClone = NULL;

for (ntPos = ntContig->cloneList; ntPos != NULL; ntPos = ntPos->next)
    {
    clone = ntPos->clone;
    if (lastClone != NULL)
        {
	diffOne = clone->mapPos - lastClone->mapPos;
	diffTotal += diffOne;
	}
    lastClone = clone;
    }
...
if (edge == NULL)
    ...
    if (bridge->maxDistance > maxDistance)
        bridge->maxDistance = maxDistance;
    if (bridge->minDistance > bridge->maxDistance)
         {
	 uglyf("Bridging between %s and %s type %s\n", aName, bName, 
	    cableTypeName(type));
	 uglyf("bridge->minDistance %d, bridge->maxDistance %d\n",
	 	bridge->minDistance, bridge->maxDistance);
	 }
...
struct lineGraph *makeMrnaLines(char *fileName, struct hash *fragHash,
	enum cableType cableType, int scoreAdjust, struct hash *ntCloneHash,
...
pslList = loadPslAndNtFixTarget(fileName, ntCloneHash, splitFinFragHash);
...
while ((wordCount = lineFileChop(lf, words)) != 0)
    ...
    if (hashLookup(ntCloneHash, cloneName))
        {
	warn("%s is part of an NT contig, skipping chain info", cloneName);
	continue;
	}
...
struct lineGraph *makePairedLines(struct pair *pairList, char *pslName, 
	...
	struct hash *ntCloneHash, struct hash *splitFinFragHash, int maxMapDistance)
...
pslList = loadPslAndNtFixTarget(pslName, ntCloneHash, splitFinFragHash);
for (psl = pslList; psl != NULL; psl = nextPsl)
    {
    nextPsl = psl->next;
    if (pslTargetFragFilter(psl, fragHash))
...
void processSelf( struct oogClone **pCloneList,
...
for (psl = *pSelfPslList; psl != NULL; psl = nextPsl)
    {
    nextPsl = psl->next;
...
for (ref = refList; ref != NULL; ref = ref->next)
    ...
    else if (sameString(psl->tName, aFrag->name))
	{
...
struct overlappingClonePair *findCloseHashedOcp(char *aName, char *bName, struct hash *ocpHash)
/* Find overlappingClonePair if any associated with a/b overlap that is within
 * maxMapDeviation on map. */
{
char *ocpName = ocpHashName(aName, bName);
struct overlappingClonePair *ocp = hashFindVal(ocpHash, ocpName);
int dif;
...
if (ocp != NULL)
    {
    dif = maxMapDif(ocp->a, ocp->b);
    if (dif < maxMapDeviation)
        return ocp;
    else
        return NULL;
    }
...
int findCloseHashedOverlap(char *aName, char *bName, struct hash *ocpHash)
/* Find how much a and b overlap if they are within maxMapDeviation on map. */
{
struct overlappingClonePair *ocp;
ocp = findCloseHashedOcp(aName, bName, ocpHash);
if (ocp == NULL)
   return 0;
return ocp->overlap;
}
...
int findCloseNodeOverlap(struct dlNode *a, struct dlNode *b, struct hash *ocpHash)
/* Find how much a and b overlap. */
{
struct bargeEl *aBel = a->val, *bBel = b->val;
char *aName = aBel->clone->name, *bName = bBel->clone->name;
return findCloseHashedOverlap(aName, bName, ocpHash);
}

boolean placeClone(struct dlList *orderedList, struct dlNode *cNode,
	struct hash *ocpHash)
/* Place clone c that overlaps with ordered list onto ordered list.
...
c = cNode->val;
	...
	a = aNode->val;
	fprintf(logFile, "  %s  %d\n", a->clone->name, 
		findCloseNodeOverlap(aNode, cNode, ocpHash));
	bNode = aNode->next;
	if (bNode->next != NULL)
	    {
	    b = aNode->val;
...
for (aNode = orderedList->head; aNode->next != NULL; aNode = aNode->next)
    ...
    a = aNode->val;
    b = bNode->val;
    if ((acOverlap = findCloseNodeOverlap(aNode, cNode, ocpHash)) == 0)
	continue;
    if (bNode->next == NULL)
	{
    ...
    if ((bcOverlap = findCloseNodeOverlap(bNode, cNode, ocpHash)) == 0)
	{
...
struct overlappingClonePair *ocp;
int overlap;
struct bargeEl *oBel, *rBel;
int dif;
struct oogClone *oClo, *rClo;
...
for (oNode = orderedList->head; oNode->next != NULL; oNode = oNode->next)
	...
	rBel = rNode->val;
	rClo = rBel->clone;
	ocp = findHashedOcp(oClo->name, rClo->name, ocpHash);
	if (ocp != NULL)
	    {
	    overlap = ocp->overlap;
	    dif = maxMapDif(oClo, rClo);
	    if (dif < maxMapDeviation)
		{
		dif -= freeMapDeviation;
		if (dif > 0)
		    {
		    int devRange = maxMapDeviation - freeMapDeviation;
		    double scale = devRange - dif;
		    scale /= devRange;
		    overlap = round(scale*overlap);
		    }
...
struct barge *makeLiteralBargeList(struct oogClone *cloneList, struct hash *ocpHash)
/* Create a list of barges that slavishly follows map coordinates. */
{
struct barge *bargeList = NULL, *barge;
struct oogClone *lastClone = NULL, *clone;
struct bargeEl *bel;
int firstClonePos = 0;

for (clone = cloneList; clone != NULL; clone = clone->next)
    {
    if (lastClone == NULL || findHashedOverlap(lastClone->name, clone->name, ocpHash) <= 0)
        {
	AllocVar(barge);
	slAddHead(&bargeList, barge);
        barge->id = nextBargeId();
	firstClonePos = clone->mapPos;
	}
    AllocVar(bel);
    bel->clone = clone;
    clone->bel = bel;
    clone->barge = barge;
    bel->offset = clone->mapPos - firstClonePos;
    slAddHead(&barge->cloneList, bel);
    lastClone = clone;
    }
...
void outcastsToEnclosed(struct dlList *outcastList, struct dlList *enclosedList, 
     float threshold)
/* Move outcasts that are mostly enclosed to enclosed list. */
{
struct dlNode *aNode, *bNode, *nextNode;
struct bargeEl *aBel, *bBel, *e;
struct oogClone *aClone, *bClone;

for (aNode = outcastList->head; aNode->next != NULL; aNode = nextNode)
    {
    nextNode = aNode->next;
    aBel = aNode->val;
    aClone = aBel->clone;
    bClone = aClone->maxShareClone;

    if (bClone != NULL)
	{
	/* Check to avoid loops in enclosure tree. */
	for (e = bClone->bel; e != NULL; e = e->enclosed)
	    {
	    if (e->clone == aClone)
		{
		fprintf(logFile, "Avoiding circular enclosure between %s and %s\n", aClone->name, bClone->name);
		bClone = NULL;
		break;
		}
	    }
    ...
    if (bClone != NULL && aClone->maxShare >= round(threshold * aClone->size))
        {
	bBel = bClone->bel;
	aBel->enclosed = bBel;
	fprintf(logFile, "Outcast %s enclosed 80%% by %s\n", aClone->name, bClone->name);
	dlRemove(aNode);
	dlAddTail(enclosedList, aNode);
	recordContamination(aClone->name, "mixedPlate");
	}
    }
}

struct barge *makeBargeList(struct oogClone *cloneList, struct raft *raftList, 
	struct hash *ocpHash, struct overlappingClonePair *ocpList)
...
struct barge *makeBarges(struct oogClone *cloneList, struct hash *cloneHash,
	struct raft *raftList, struct hash *ocpHash, boolean literalMap)
/* Make barges including ordering clones inside them. */
{
struct barge *bargeList, *barge;
struct raft *raft;
struct oogClone *clone;
struct overlappingClonePair *ocpList;

ocpList = findOverlappingClones(cloneList, raftList, ocpHash);
saveOcp(ocpFileName, ocpList);
findMaxShare(ocpList);

if (literalMap)
    bargeList = makeLiteralBargeList(cloneList, ocpHash);
else
    bargeList = makeBargeList(cloneList, raftList, ocpHash, ocpList);
...
for (barge = bargeList; barge != NULL; barge = barge->next)
	...
	if (clone->ntContig)
	    {
	    struct ntContig *ntContig = clone->ntContig;
	    struct ntClonePos *pos, *nextPos;
	    struct oogClone *subClone;
	    int orientation = ntContigWeightedOrientation(ntContig);

	    if (orientation < 0)
		slReverse(&ntContig->cloneList);
	    for (pos = ntContig->cloneList; pos != NULL; pos = nextPos)
		{
		int posEnd, posStart, nextPosStart;
		nextPos = pos->next;
		subClone = pos->clone;
		if (orientation < 0)
		    posStart = ntContig->size - (pos->pos + pos->size);
		else
		    posStart = pos->pos;
		if (nextPos)
		    {
		    if (orientation < 0)
		        {
			posEnd = ntContig->size - pos->pos;
			nextPosStart = ntContig->size - (nextPos->pos + nextPos->size);
			}
		    else
		        {
			posEnd = pos->pos + pos->size;
			nextPosStart = nextPos->pos;
			}
		    overlap = (posEnd - nextPosStart);
		    }
		else
		    {
		    overlap = 0;
		...
		fprintf(f, "%7d %-9s %6d %6d   %c       %s\n",
		    posStart + clone->defaultPos, pos->name, pos->size, overlap,
		    orientChar(orientation), clone->name);
		}
	    if (orientation < 0)
		slReverse(&ntContig->cloneList);
	    }
...
void loadAllClones(char *infoName, char *genoList, 
	...
	char **retCtgName, char **retInfoType)
...
for (i=0; i<genoCount; ++i)
	...
	if ((hel = hashLookup(cloneHash, acc)) != NULL)
	    ...
	    if ((ntPos = hashFindVal(ntCloneHash, acc)) != NULL)
		{
		int fragCount = dlCount(clone->fragList);
		ntPos->clone = clone;
		if (fragCount != 1)
		    warn("Finished NT clone %s is in %d pieces", 
		    	clone->name,  fragCount);
		}
	    else
	        {
		slAddHead(pCloneList, clone);
...
*retInfoType = cloneString(words[1]);
while (lineFileNext(lf, &line, &lineSize))
    {
    if ((wordCount = chopLine(line, words)) < 2)
...
struct ntContig *ntContig;
struct oogFrag *aFrag, *bFrag;
int diffInMap;

logIt("calcRaftFlipTendency %d\n", raft->id);	/* uglyf */
for (rf = raft->fragList; rf != NULL; rf = rf->next)
    {
	...
	if (intAbs(diffInMap) < maxMapDeviation)
	    {
	    logIt("  %d %d, diffOne %d\n", rfNext->frag->defaultPos, rf->frag->defaultPos, diffOne);	/* uglyf */
	    diffTotal += diffInMap;
	    }
	else
	    {
    ...
    if ((ntContig = rf->frag->ntContig) != NULL)
        {
	diffOne = ntContigWeightedOrientation(ntContig);
	logIt("  NT diffOne %d\n", diffOne);	/* uglyf */
	if (rf->orientation < 0)
	    diffOne = -diffOne;
	diffTotal += 4*diffOne;
...
void splitFinCloneIntoGp(struct splitClone *splitClone, 
	struct ntClonePos *pos, int start, int end, int orientation,
	struct goldenPath **pGpList)
/* Output segments from a finished clone that is in more than one piece
 * to golden path. */
{
struct oogClone *clone = pos->clone;
struct dlNode *node = clone->fragList->head;
struct goldenPath *gp;
struct splitFrag *frag, *lastFragIn = NULL;
int s, e;

for (frag = splitClone->fragList; frag != NULL; frag = frag->next)
    {
    if (dlEnd(node))
       errAbort("More fragments in splitFin than in %s.fa", clone->name);
    s = frag->start + pos->pos;
    e = frag->end + pos->pos;
    if (s < start) s = start;
    if (e > end) e = end;
    if (s < e)
	...
	if (lastFragIn != NULL)
	    ...
	    AllocVar(gp);
	    gp->fragStart = 0;
	    gp->fragEnd = frag->start - lastFragIn->end;
	    gp->gapType = gtSplitFin;
	    gp->bridgeType = btYes;
	    slAddHead(pGpList, gp);
	    }
	AllocVar(gp);
	gp->frag = node->val;
	if (!sameString(gp->frag->name, frag->name))
	    errAbort("splitFin and %s.fa are out of sync at %s", clone->name, frag->name);
	gp->fragStart = s - pos->pos - frag->start;
	gp->fragEnd = e - pos->pos - frag->start;
	gp->orientation = pos->orientation*orientation;
	slAddHead(pGpList, gp);
	lastFragIn = frag;
	}
    node = node->next;
...
void addNtFragsToGp(struct ntContig *ntContig, struct hash *splitFinCloneHash,
	int start, int end, int orientation, struct goldenPath **pGpList)
/* Add parts of NT contig from start to end to pGpList.  */
{
struct ntClonePos *pos, *nextPos;
struct goldenPath *lPath = NULL;
int s, e;
struct goldenPath *gp, *nextGp;
struct goldenPath *revGpList = NULL;
struct goldenPath **pActive = (orientation > 0 ? pGpList : &revGpList);
struct oogClone *clone;
struct dlNode *node;
...
for (pos = ntContig->cloneList; pos != NULL; pos = nextPos)
    ...
    nextPos = pos->next;
    if (pos->orientation < 0)
	warn("Reversed orientation in ntContig %s clone %s. Don't know if ooGreedy can handle it...", 
	    ntContig->name, pos->name);

    /* Find intersection between this clone of NT and the NT fragment
     * we're outputting. */
    s = pos->pos;
    e = s + pos->size;
    if (nextPos != NULL && nextPos->pos < e)
	e = (e + nextPos->pos)/2; /* Split overlap with next clone. */
    if (s < start) s = start;
    if (e > end) e = end;
    if (s < e)
	{
	/* We need to output parts of this clone. */
	clone = pos->clone;
	if (dlCount(clone->fragList) > 1)
	    {
	    /* If the clones is split, our life gets a little complicated. */
	    struct splitClone *splitClone = findHashValOrDie(splitFinCloneHash, 
	    	pos->name, "Multi-frag clone");
	    splitFinCloneIntoGp(splitClone, pos, s, e, orientation, pActive);
	    }
	else  /* Simple case - only one fragment. */
	    {
	    AllocVar(gp);
	    gp->fragStart = s - pos->pos;
	    gp->fragEnd = e - pos->pos;
	    gp->orientation = pos->orientation*orientation;
	    node = clone->fragList->head;
	    gp->frag = node->val;
	    slAddHead(pActive, gp);
	    }
	start = e;
	}
    }
if (orientation < 0)
    {
    uglyf("Reversing gp through NT %s\n", ntContig->name);
    for (gp = revGpList; gp != NULL; gp = nextGp)
	{
	nextGp = gp->next;
	slAddHead(pGpList, gp);
	}
...
struct goldenPath *expandPathThroughNtContigs(struct goldenPath *gpList,
	struct hash *splitFinCloneHash)
/* Return path where frags that hit an NT contig are broken into
 * fragments from the appropriate NT clones. */
{
struct goldenPath *gp, *nextGp, *newList = NULL;
struct oogFrag *frag;

for (gp = gpList; gp != NULL; gp = nextGp)
    {
    nextGp = gp->next;
    frag = gp->frag;
    if (frag->ntContig)
        {
	addNtFragsToGp(frag->ntContig, splitFinCloneHash, gp->fragStart, 
		gp->fragEnd, gp->orientation, &newList);
	freez(&gp);
	}
    else
        {
...
checkGp(&gp);
gp = expandPathThroughNtContigs(gp, splitFinCloneHash);
return gp;
}
...
struct hash *ntContigHash = newHash(7); /* An entry for each ntContig. */
struct hash *ntCloneHash = newHash(9);  /* ntClonePos for each clone of each ntContig. */
...
struct pair *readPairList;
struct pair *bacPairList;
struct raft *raftList, *raft, *oneRaftSet;
...
if (fileExists(ntName))
    {
    ntList = readNtFile(ntName, ntContigHash, ntCloneHash);
    status("Got %d NT contigs\n", slCount(ntList));
    }
...
loadAllClones(infoName, genoListName, fragHash, 
	cloneHash, ntCloneHash, &fragList, &cloneList, &ctgName, &infoType);
sprintf(faOutName, "%s/%s.fa", inDir, ctgName);
checkNtClonesLoaded(ntList);
removeNtOrigFrags(ntList, &fragList, &ntOrigFragList);
addNtPsuedoClones(ntList, fragHash, &fragList, cloneHash, &cloneList);
...
/* Figure out how much to believe map. */
if (sameWord(infoType, "PLACED"))
    literalMap = TRUE;
else if (sameWord(infoType, "RANDOM"))
    ;
else if (sameWord(infoType, "ORDERED"))
    ;
else
    errAbort("Unrecognized type '%s' in info file", infoType);
    
/* Load up and process self edge list, converting NT clones to
 * NT contigs, and then psl's to edges. */
    {
    int oCount;
    selfPslList = pslLoadAll(selfPslName);
    oCount = slCount(selfPslList);
    ntPslTranslate(selfPslList, ntCloneHash, splitFinFragHash, TRUE, TRUE);
    ntPslMergeOverlapping(&selfPslList, FALSE);
    ntPslMergeOverlapping(&selfPslList, TRUE);
    pslWriteAll(selfPslList, "selfNt.psl", TRUE);	/* uglyf */
    status("%d self psls before, %d after merging\n", oCount, 
    	slCount(selfPslList));
    processSelf( &cloneList, &fragList, &selfEdgeList, &selfPslList, cloneHash, fragHash);
    status("Found %d fragment overlaps in self.psl\n", slCount(selfEdgeList));
    slSort(&selfEdgeList, oogEdgeCmp);
    }


/* Make initial rafts from which we will derive barges. */
raftList = makeRafts(fragList, selfEdgeList, TRUE);
status("found %d initial rafts (%d of %d frags) in %s\n", 
	slCount(raftList), raftedFragCount(fragList),
	slCount(fragList), selfPslName);

/* Make barges. */
bargeList = makeBarges(cloneList, cloneHash, raftList, ocpHash, literalMap);
status(" %d barges in %d clones\n", slCount(bargeList), slCount(cloneList));

/* Remake rafts forcing them to fit on barges. */
unlinkFragsFromRafts(fragList);
raftFreeList(&raftList);
edgesIntoBarges(bargeList, &selfEdgeList);
 ...
 * merge them into one big list sorted by score. */  
    ...
    bacEndLineList = makePairedLines(bacPairList, bacPslName, 
    	fragHash, bacPairHash, ctBacEndPair, ntCloneHash,
