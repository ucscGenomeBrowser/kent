...
for (;;)
    ...
    fprintf(out, "Alt splicings of %s in %s\n", baseGeneName, gffName);
...
char *argv[];
{
    ...
    mg = mgNew(100,100);
    mgClearPixels(mg);
    mgDrawBox(mg, 25, 25, 50, 50, MG_BLACK);
    mgDrawLine(mg, 0, 0, 100, 100, MG_RED);
    mgDrawLine(mg, 0, 100, 100, 0, MG_BLUE);
    mgDrawLine(mg, -20, 120, 90, 40, MG_GREEN);
    mgSaveGif(mg, "s.gif");
    mgFree(&mg);
}
