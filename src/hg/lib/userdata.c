...
/* Fix up multiple '/', './', '..' chars in a path name without checking
 * that each component must exist. This was mostly generated by ChatGPT */
{
if (path == NULL)
    return NULL;
...
char *parts[1024]; // max components
int count = 0;
...
size_t len = 2; // at least '/'
for (int i = 0; i < count; i++)
    {
    len += strlen(parts[i]) + 1;
    }

char *normalized = needMem(len);
if (!normalized)
    {
    free(copy);
    return NULL;
...
if (!tusdDataBaseDir  || isEmpty(tusdDataBaseDir))
    errAbort("No tusdDataDir defined in hg.conf");
if (tusdDataBaseDir[0] != '/')
    errAbort("config setting tusdDataDir must be an absolute path (starting with '/')");
// the tusdDataDir must be resolved in case it is a symlink
char resolvedDataDir[PATH_MAX];
realpath(tusdDataBaseDir, resolvedDataDir);
if (isEmpty(resolvedDataDir))
    errAbort("Error resolving hg.conf:tusdDataDir. Ensure that setting points to a valid location");

tusdDataBaseDir = resolvedDataDir;

struct dyString *newDataDir = dyStringNew(0);
dyStringPrintf(newDataDir, "%s/", tusdDataBaseDir);
if (serverName)
    dyStringPrintf(newDataDir, "%s/", serverName);
dyStringPrintf(newDataDir, "%s", getEncodedUserNamePath(userName));

// don't use realpath here as the userPrefix and serverName directory
// may not have been created yet
char *canonicalPath = normalizePath(dyStringContents(newDataDir));
...
char *getHubDataDir(char *userName, char *hub, char *serverName)
{
char *dataDir = getDataDir(userName, serverName);
...
if (userName)
    ...
    dyStringPrintf(userDirDy, "%s/%s/%s/%s/", getHubSpaceUrl(), serverName, userPrefix, encUserName);
...
{
char *webDataUrl = webDataDir(userName, serverName);
if (webDataUrl)
    {
...
{
char *pathPrefix = getDataDir(userName, serverName);
char *path = NULL;
if (pathPrefix)
    {
    if (parentDir)
...
if (!startsWith(getDataDir(userName, serverName), canonicalPath))
    return;
