/* cdwWebSubmit - A small self-contained CGI for submitting data to the CIRM Data Warehouse.. */
...
errAbort(
  "cdwWebSubmit - A small self-contained CGI for submitting data to the CIRM Data Warehouse.\n"
  "usage:\n"
  "   cdwWebSubmit cgiVar=value cgiVar2=value2\n"
  );
}
...
{
printf("<DIV>");
printf("<H3>Welcome to the CIRM data submission site</H3>");
...
struct cdwSubmit *sub = cdwMostRecentSubmission(conn, url);
time_t startTime = 0, endTime = 0, endUploadTime = 0;
if (sub == NULL)
    {
    int posInQueue = cdwSubmitPositionInQueue(conn, url, NULL);
    if (posInQueue == 0)
    ...
    endTime = (endUploadTime ? endUploadTime : cdwNow());
    ...
    struct cdwUser *user = cdwUserFromId(conn, sub->userId);
	...
	if (endUploadTime == 0)
	    {
	    struct cdwFile *ef = cdwFileInProgress(conn, sub->id);
	    if (ef != NULL)
		{
		char path[PATH_LEN];
	...
	struct dyString *duration = cdwFormatDuration(timeSpan);
	...
	if (endUploadTime == 0 && timeSpan > 0)
		...
		struct dyString *eta = cdwFormatDuration(estimatedFinish);
...
if (autoRefresh && isEmpty(sub->errorMessage))
    ...
    cdwWebAutoRefresh(CDW_WEB_REFRESH_5_SEC);
    }
}
...
if (cdwUserIsAdmin(conn,userEmail)) 
    return TRUE; 
if (!sqlRowExists(conn, "cdwSubmit", "url", url))
    return TRUE;
/* url exists, find out who is the original submitter */
int uId = cdwFindUserIdFromEmail(conn, userEmail);
char  query[256];
sqlSafef(query, sizeof(query),
    "select distinct userId from cdwSubmit where url='%s'", url);
...
cdwMustGetUserFromEmail(conn, userEmail);
...
cdwAddSubmitJob(conn, userEmail, url, cgiBoolean("update"));
}
...
if (sub == NULL)
    ...
    unsigned cdwSubmitJobId = 0;
    int posInQueue = cdwSubmitPositionInQueue(conn, url, &cdwSubmitJobId);
    if (posInQueue >= 0)
        {
	char query[256];
	sqlSafef(query, sizeof(query), "delete from cdwSubmitJob where id=%u", cdwSubmitJobId);
    ...
    char query[256];
    sqlSafef(query, sizeof(query), 
	"update cdwSubmit set errorMessage='Stopped by user.' where id=%u", sub->id);
    sqlUpdate(conn, query);
    }
}
...
cdwWebSubmitMenuItem(TRUE);
printf("<FORM ACTION=\"../cgi-bin/cdwWebSubmit\" METHOD=GET>\n");
struct sqlConnection *conn = cdwConnectReadWrite(cdwDatabase);
