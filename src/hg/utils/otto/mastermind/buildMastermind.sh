...
set -eE -o pipefail
# Emit an error line on any failure so cron actually sends mail. wget -q is silent and
# set -e would otherwise abort with no output, which cron treats as nothing to report.
# -E (errtrace) makes the ERR trap fire for failures inside functions/subshells too.
trap 'echo "ERROR: mastermind build failed (exit $?) at line $LINENO" >&2' ERR
...
curl -sf https://mastermind.genomenon.com/cvr/version | cut -d\" -f4 > mastermindRelease.new.txt
