/* hgSubj - A CGI script to display the subject details page.. */
#include "common.h"
#include "linefile.h"
#include "hash.h"
#include "jksql.h"
...
errAbort(
  "hgSubj - A CGI script to display the subject details page.\n"
  "usage:\n"
  "   hgSubj cgi-vars in var=val format\n"
  "options:\n"
...
static char *rootDir = "hgSubjData";
...
puts("<FORM ACTION=\"/cgi-bin/hgSubj\" NAME=\"mainForm\" METHOD=\"GET\">\n");
...
if (sameWord(curSubjId, ""))
    ...
    printf("</H3>");
    printf("For example: test_GSID3123");fflush(stdout);
    }
else
    {
    ...
    if (row != NULL)
    	...
    	printf("<input type=\"text\" name=\"hgs_subj\" value=\"%s\">\n", "");
    	cgiMakeButton("submit", "Go!");
    	printf("<BR><BR>For example: test_GSID3123");
	printf("</H3>");
	fflush(stdout);
	}
    }
