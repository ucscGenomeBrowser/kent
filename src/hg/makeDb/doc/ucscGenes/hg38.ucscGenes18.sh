...
export dir=/cluster/data/hg38/bed/ucsc.18.1
export GENCODE_VERSION=V26
export oldGeneDir=/cluster/data/hg38/bed/ucsc.17.1
export oldGeneBed=$oldGeneDir/ucscGenes.bed
export db=hg38
export spDb=sp170510
export taxon=9606
export tempDb=tmpFoo77
...
echo 5003138 > startId
txGeneAccession -test $oldGeneBed startId gencode${GENCODE_VERSION}Comp.bed.gz txToAcc.tab oldToNew.tab
cat startId
# 5008279
...
twoBitToFa -noMask /cluster/data/$db/$db.2bit -bed=ucscGenes.bed ucscGenes.fa
...
hgsql $tempDb -Ne "select k.name, g.geneId, g.geneStatus, g.geneType,g.transcriptName,g.transcriptType,g.transcriptStatus, g.havanaGeneId,  g.ccdsId, g.level, g.transcriptClass from knownGene k, hg38.wgEncodeGencodeAttrs$GENCODE_VERSION g where k.alignID=g.transcriptId" > knownAttrs.tab
