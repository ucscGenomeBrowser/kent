...
my %genbankToUcsc;	# key is genbank name, value is ucsc name
my %ucscToGenbank;	# key is ucsc name, value is genbank name

open (FH, "hgsql -N -e 'select alias,chrom from chromAlias where source like \"genbank%\";' hg19|") or die "can not select from hg19.chromAlias";
while (my $line = <FH>) {
  chomp $line;
  my ($genbank, $ucsc) = split('\s+', $line);
  die "ERROR: ambiguous genbank to ucsc name $genbank $ucsc != $genbankToUcsc{$genbank}" if (exists($genbankToUcsc{$genbank}));
  $genbankToUcsc{$genbank} = $ucsc;
  die "ERROR: ambiguous ucsc to genbank name $ucsc $genbank != $ucscToGenbank{$ucsc}" if (exists($ucscToGenbank{$ucsc}));
  $ucscToGenbank{$ucsc} = $genbank;
}
close (FH);
...
while (my $line = <FH>) {
  ...
  $chr =~ s/.HUMMTC//;
  $patchSize{$chr} = $size;
#  printf STDERR "patchSize{$chr} = $size\n";
}
close (FH);
...
while (my $line = <FH>) {
   ...
   my $ucscAccession = lc($accession);
   $ucscAccession =~ s/.[1-9]$//;
   die "can not find patchSize{$accession}" if (! defined($patchSize));
   if ($a[1] =~ m/alt-scaffold/) { $ucscName = "chr${chrN}_${ucscAccession}_alt"; }
   elsif ($a[1] =~ m/unlocalized-scaffold/) { $ucscName = "chr${chrN}_${ucscAccession}_random"; }
   elsif ($a[1] =~ m/assembled-molecule/) { $ucscName = "chr${chrN}"; }
   elsif ($a[1] =~ m/unplaced-scaffold/) { $ucscName = "chrUn_${ucscAccession}"; }
   elsif ($a[1] =~ m/fix-patch/) { $ucscName = "chr${chrN}_${ucscAccession}_fix"; }
   elsif ($a[1] =~ m/novel-patch/) { $ucscName = "chr${chrN}_${ucscAccession}_alt"; }
   else { die "do not recognize type: '$a[1]'";
   }
   $ucscName = $genbankToUcsc{$accession} if (defined($genbankToUcsc{$accession}));
   my $warnings = "";
