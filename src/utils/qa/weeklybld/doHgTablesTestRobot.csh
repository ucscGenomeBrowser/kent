#!/bin/tcsh
cd $WEEKLYBLD

cd $BUILDDIR/v${BRANCHNN}_branch/kent/src/hg/hgTablesTest
make 
cd $WEEKLYBLD

setenv HGDB_CONF /cluster/home/build/.hg.conf.beta

# Run the binary the make above just produced.  userApp.mk installs to
# $HOME/bin/$MACHTYPE, so name that path rather than a shared directory this
# robot does not own.
set hgTablesTest = $HOME/bin/$MACHTYPE/hgTablesTest
if ( ! -x $hgTablesTest ) then
    echo "ERROR: $hgTablesTest missing or not executable - did the make above fail?"
    echo "$hgTablesTest missing or not executable; the make above probably failed." \
      | mail -s "Errors in hgTablesTestRobot on $HOST" ${BUILDMEISTEREMAIL}
    exit 1
endif

set log = v${BRANCHNN}.hgTables.log
# Combined stdout/stderr of the two hgTablesTest runs.  Kept SEPARATE from $log:
# hgTablesTest writes its structured report to $log itself and ALSO echoes the
# summary to stdout, so merging the two streams into one file would double every
# "Total:" line and defeat the completed-run count below.
set runout = v${BRANCHNN}.hgTables.runout

# Who hears about the result.  Defaults to the build meister plus QA; a
# diagnostic re-run can narrow it with e.g.
#   setenv ROBOT_MAILTO build@soe.ucsc.edu
# so re-testing does not notify the QA list.
set mailto = "${BUILDMEISTEREMAIL} browser-qa@soe.ucsc.edu"
if ( $?ROBOT_MAILTO ) then
    set mailto = "$ROBOT_MAILTO"
endif

rm -f ./logs/$runout
echo "$hgTablesTest -db=hg38 https://hgwbeta.soe.ucsc.edu/cgi-bin/hgTables ./logs/$log" > ./logs/$log
$hgTablesTest -appendLog -db=hg38 https://hgwbeta.soe.ucsc.edu/cgi-bin/hgTables ./logs/$log >>& ./logs/$runout
set st1 = $status
echo "" >> ./logs/$log
echo "$hgTablesTest -appendLog -org=Mouse -orgs=1  https://hgwbeta.soe.ucsc.edu/cgi-bin/hgTables ./logs/$log" >> ./logs/$log
$hgTablesTest -appendLog -org=Mouse -orgs=1  https://hgwbeta.soe.ucsc.edu/cgi-bin/hgTables ./logs/$log >>& ./logs/$runout
set st2 = $status

# creates hgTables.log - look for unusual errors

#-- to check for errors:
# hgTablesTest prints its "Total: N tests, N soft errors, N hard errors" line
# from reportSummary() only after a run finishes.  When a run dies partway --
# the carefulAlloc 500MB ceiling, a segfault, an errAbort it has no guard for --
# that line never gets written.  The old check looked ONLY for Total lines that
# carried errors, so a crash produced an empty match and was reported as
# success.  So now a missing Total summary, or a nonzero exit from either run,
# is itself a hard failure.  This mirrors preview2TablesTestRobot.csh, refs #38356.
set expectedRuns = 2
set nTotal = `egrep -c "^[ 	]*Total:" ./logs/$log`
set res  = `egrep "^[ 	]*Total:" ./logs/$log | egrep -v "0 soft errors,  0 hard errors"`
set res2 = `awk '/^[ \t]*Total/ {print} /TablesTest/ {print}' ./logs/$log`
set crash = `cat ./logs/$log ./logs/$runout | egrep "carefulAlloc|Couldn.t |needLargeMem|needMem is null|Segmentation|Out of memory|Assertion|no form produced"`

set problems = ""
if ( "$st1" != "0" ) then
    set problems = "$problems hg38-run-exited-$st1"
endif
if ( "$st2" != "0" ) then
    set problems = "$problems mouse-run-exited-$st2"
endif
if ( "$nTotal" != "$expectedRuns" ) then
    set problems = "$problems incomplete-only-$nTotal-of-$expectedRuns-runs-reached-their-Total-summary"
endif
if ( "$res" != "" ) then
    set problems = "$problems errors-reported-in-Total-summary"
endif
if ( "$crash" != "" ) then
    set problems = "$problems crash-signature-in-log"
endif

if ( "$problems" != "" ) then
    echo "errs found:$problems"
    echo "$res2"
    ( echo "hgTablesTest robot FAILED for v${BRANCHNN} on $HOST" ; \
      echo "" ; \
      echo "problems:$problems" ; \
      echo "" ; \
      echo "hg38 run exit status : $st1" ; \
      echo "Mouse run exit status: $st2" ; \
      echo "completed runs       : $nTotal of $expectedRuns reached their Total summary" ; \
      echo "" ; \
      echo "crash signatures:" ; \
      echo "$crash" ; \
      echo "" ; \
      echo "summary lines:" ; \
      echo "$res2" ; \
      echo "" ; \
      echo "report log: $WEEKLYBLD/logs/$log" ; \
      echo "run output: $WEEKLYBLD/logs/$runout" ) \
      | mail -s "Errors in hgTablesTestRobot on $HOST" $mailto
    exit 1
endif
#
echo Done.
echo "$res2" | mail -s "v${BRANCHNN} hgTablesTest robot done successfully on $HOST." $mailto
exit 0
