...
${ECHO} "# trashCleaner.bash: trashFile: '${trashFile}'" 1>&2
${ECHO} "# trashCleaner.bash: absSaveFile: '${absSaveFile}'" 1>&2
${ECHO} "# trashCleaner.bash: relSaveFile: '${relSaveFile}'" 1>&2
    if [ -l "${trashFile}" ]; then
...
${ECHO} "# dbTrashLog: ${dbTrashLog}"
${ECHO} "after dbTrash ${machName} cleaner "`$dateCmd`
/bin/gzip -f "${dbTrashLog}"
...
/bin/gzip -f ${rmLogDir}/shortTimeList.${dateStamp}.txt
...
# XXX  output routed to null for testing
${egrepCmd} "noMatchToNothing|^$trashDir" ${TMPDIR}/${machName}.shortTime.list.txt | /bin/sed -e "/noMatchToNothing/d; s/'/\\\'/" | xargs --no-run-if-empty ${rmCmd} > /dev/null
...
/bin/gzip -f ${rmLogDir}/longTimeList.${dateStamp}.txt
...
/bin/rm "${refreshList}"
/bin/rm "${shortTimeAtimeFile}" "${longTimeAtimeFile}"
if [ -e "${atimeFile}" ]; then
  /bin/rm "${atimeFile}"
fi
