...
if [ -z "${SCRIPTS+x}" ]; then
  ${ECHO} "ERROR: trashSizeMonitor.sh needs to have SCRIPTS defined" 1>&2
  exit 255
fi
if [ -z "${LOGDIR+x}" ]; then
...
# make sure at least one trash file exists
...
$SCRIPTS/fileStatsFromFind.pl "${trashFileList}"
export dirList=`sed -e 's#.*/trash/##;' "${trashFileList}" | grep "/" | sed -e 's#/.*##' | sort -u | xargs ${ECHO}`
$SCRIPTS/dirStatsFromFind.pl "${trashFileList}" $dirList
