/* GbToFaRa - Takes GenBank flat format files and extracts only
 * the mRNA sequence for humans.  It puts the sequence in a .fa
...
/* Controls which records are chosen and which fields are written. */
    {
    struct keyExp *keyExp;
    struct slName *hideKeys;
    };
...
slAddTail(&gbs, c0);
    authorsField = c1 = newGbField("AUTHORS", "aut", TRUE, FALSE, 256);
    slAddTail(&c0->children, c1);

    journalField = c1 = newGbField("JOURNAL", NULL, FALSE, TRUE, 256);
    slAddTail(&c0->children, c1);

commentField = c0 = newGbField("COMMENT", "com", FALSE, TRUE, 4*1024);
slAddTail(&gbs, c0);
...
for (;;)
    ...
    if (!isdigit(s[0]))
        break;
    start = atoi(s);
    if ((s = nextWord(s)) == NULL || !isdigit(s[0]))
    ...
    if ((s = nextWord(s)) == NULL || !startsWith("of", s) )
...
void bacWrite(char *faDir, char *accession, DNA *dna, int dnaSize)
...
while (readGbInfo(lf))
    ...
    if (locus == NULL || accession == NULL)
        errAbort("No LOCUS or no ACCESSION line near %d of %s", lf->lineCount, inName);
    ...
    if (!hashLookup(uniqueNameHash, accession))
        ...
        if (wordCount >= 6)
            ...
            kvtAdd(kvt, "cat", words[4]);
            kvtAdd(kvt, "dat", words[5]);
            }
        ...
        if (keyExpEval(filter->keyExp, kvt))
            ...
            if (formatType == ftBac)
                {
                bacWrite(faDir, accession, dna, dnaSize);
                }
            else
