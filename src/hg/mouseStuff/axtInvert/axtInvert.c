...
errAbort(
  "axtInvert - Swap source and query in an axt file\n"
  "usage:\n"
  "   axtInvert source.axt target.sizes query.sizes dest.axt\n"
  "options:\n"
  "   -xxx=XXX\n"
  );
}
...
void axtInvert(struct axt *axt, int tSize, int qSize)
...
void axtInvertFile(char *source, char *targetSizes, char *querySizes, char *dest)
/* axtInvert - Swap source and query in an axt file. */
{
...
while ((axt = axtRead(lf)) != NULL)
    {
    axtInvert(axt, hashIntVal(tHash, axt->tName), hashIntVal(qHash, axt->qName));
    axtWrite(axt, f);
    }
}
...
if (argc != 5)
    usage();
axtInvertFile(argv[1], argv[2], argv[3], argv[4]);
return 0;
}
