             ...
             $.ui.autocomplete,
             {
               ...
               _renderItem: function(ul, item) {
                 ...
                 // http://forum.jquery.com/topic/using-html-in-autocomplete
                   ...
                   // NCBI RefSeq "reference" assemblies get a green 'reference' word badge.
                   let refBadge = '';
                   if (item.isReference) {
                       refBadge = '<span style="color:#080;font-weight:bold;">reference</span>&nbsp;';
                   }
                   // Hits to assembly hub top level (not individial db names) have no item label,
                   // so use the value instead
                   var $a = $("<a></a>").html(clockIcon + refBadge + (item.label !== null ? item.label : item.value));
                   var $li = $("<li></li>")
                       .data("ui-autocomplete-item", item)
                       .append($a)
                       .appendTo(ul);
                   if (item.isReference && typeof addMouseover === "function") {
                       // Apply the site's positioned tooltip (utils.js) to the whole row, not just
                       // the badge glyph, and use it instead of the native title popup, which can
                       // render below the icon and get hidden under a large custom cursor -- same
                       // fix used for hgTracks image mouseovers.
                       addMouseover($a[0], "NCBI RefSeq reference assembly");
                   }
                   return $li;
               }
             });
