...
void printOneRep(struct sqlConnection *conn, struct eapGraph *eg,
    struct fullExperiment *exp, struct replicate *rep, FILE *f)
{
long long readCount = 0, mappedCount = 0, uniqueMappedCount = 0;
...
fprintf(f, "<TH>enrich</TH>");
fprintf(f, "<TH>&nbsp; </TH>");
fprintf(f, "<TH>map1</TH>");
fprintf(f, "<TH>umap1</TH>");
fprintf(f, "<TH>U4M</TH>");
fprintf(f, "<TH>&nbsp; </TH>");
fprintf(f, "<TH>map2</TH>");
fprintf(f, "<TH>umap2</TH>");
fprintf(f, "<TH>U4M</TH>");
fprintf(f, "</TR>\n");
struct fullExperiment *exp;
for (exp = expList; exp != NULL; exp = exp->next)
    ...
    fprintf(f, "<TD>&nbsp;</TD>\n");
    printOneRep(conn, eg, exp, mustFindReplicateInList(exp->repList, "1"), f);
    fprintf(f, "<TH>&nbsp; </TH>");
    printOneRep(conn, eg, exp, mustFindReplicateInList(exp->repList, "2"), f);
    fprintf(f, "</TR>\n");
    }
fprintf(f, "</TABLE>\n");
htmEnd(f);
