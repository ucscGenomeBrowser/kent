 ...
 * constructor
 */
function wfSpecialUserloginUCSC( $par = '' ) {
	global $wgRequest;
	if( session_id() == '' ) {
		wfSetupSession();
	}
...
class LoginForm {
	...
	function addNewAccount() {
		...
		if( $wgUser->isAnon() ) {
			...
			global $wgOut;
			$self = SpecialPage::getTitleFor( 'UserloginUCSC' );
	...
	function successfulLogin( $msg, $params, $auto = true ) {
		...
		if ( !empty( $this->mReturnTo ) ) {
			$this->returnToExternal( true, $this->mReturnTo );
		} else {
	...
	function mainLoginForm( $msg, $msgtype = 'error' ) {
		...
		$titleObj = SpecialPage::getTitleFor( 'UserloginUCSC' );
		...
		if ( $this->mType == 'signup' ) {
			...
			$template = new UserloginUCSCTemplate();
