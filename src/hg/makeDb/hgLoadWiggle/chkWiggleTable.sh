...
C=`hgsql -N --host=hgwdev.gi.ucsc.edu --user=hguser --password=hguserstuff \
	-e "describe ${TABLE};" ${DB} | \
	egrep "chromStart|span|dataRange|sumSquares" | wc -l`

if [ "${C}" -ne 4 ]; then
    echo "ERROR: table ${DB}.${TABLE} does not appear to be a wiggle table ?"
    hgsql --host=hgwdev.gi.ucsc.edu --user=hguser --password=hguserstuff \
	-e "describe ${TABLE};" ${DB}
    usage
    exit 255
fi
...
hgsql -N --host=hgwdev.gi.ucsc.edu --user=hguser --password=hguserstuff \
...
export HGDB_CONF
echo "db.host=hgwdev.gi.ucsc.edu" > "${HGDB_CONF}"
