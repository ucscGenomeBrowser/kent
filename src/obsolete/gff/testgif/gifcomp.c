...
short
gif_compress_data(min_code_size, pt, size, out)
int min_code_size;
unsigned char *pt;
long size;
FILE *out;
...
if ((prior_codes = (short*)malloc(TABLE_SIZE*sizeof(short))) == NULL)
	goto OUT;
if ((code_ids = (short*)malloc(TABLE_SIZE*sizeof(short))) == NULL)
	goto OUT;
if ((added_chars = (unsigned char*)malloc(TABLE_SIZE)) == NULL)
	goto OUT;
