...
export dir=/cluster/data/mm10/bed/ucsc.18.1
export GENCODE_VERSION=VM20
export oldGeneDir=/cluster/data/mm10/bed/ucsc.17.2
export oldGeneBed=$oldGeneDir/ucscGenes.bed
export db=mm10
export spDb=sp180404
# NCBI Taxon 10090 for mouse, 9606 for human
export taxon=10090
export tempDb=tmpFoo88
...
hgsql -Ne "create database ${db}Backup3" mm10
for i in  `cat $lastVer.table.lst`
do
echo "rename table $db.$i to ${db}Backup3.$i;" | hgsql mm10
done

for i in  `cat $curVer.table.lst`
do
echo "rename table $tempDb.$i to ${db}.$i;"  | hgsql mm10
done
...
#Completed: 9409 of 9409 jobs
#CPU time in finished jobs:    2257378s   37622.96m   627.05h   26.13d  0.072 y
#IO & Wait Time:                480318s    8005.30m   133.42h    5.56d  0.015 y
#Average job time:                 291s       4.85m     0.08h    0.00d
#Longest finished job:             364s       6.07m     0.10h    0.00d
#Submission to last job:          2821s      47.02m     0.78h    0.03d
...
#Completed: 9409 of 9409 jobs
#CPU time in finished jobs:    2038624s   33977.07m   566.28h   23.60d  0.065 y
#IO & Wait Time:                686628s   11443.80m   190.73h    7.95d  0.022 y
#Average job time:                 290s       4.83m     0.08h    0.00d
#Longest finished job:             514s       8.57m     0.14h    0.01d
#Submission to last job:          4238s      70.63m     1.18h    0.05d
...
cd $dir
makeBigKnown mm10
rm -f /gbdb/mm10/knownGeneM20.bb
ln -s `pwd`/mm10.knownGene.bb /gbdb/mm10/knownGeneM20.bb
