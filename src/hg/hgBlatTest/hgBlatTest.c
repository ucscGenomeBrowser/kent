...
if (sameWord(method,"1"))
    geneList = randomSample(db, dnaTable, dnaColumn, clRepeat);
else if (sameWord(method,"2"))
    geneList = randomSample(db, proTable, dnaColumn, clRepeat);
else
    errAbort("unknown method %s in .ra",method);

if (!geneList)
    {
    uglyf("testDb: randomSample returned empty geneList for %s.%s \n",db,dnaTable);
    return;
    }
