...
#define FLANK  20
#define ALLELE 20
#define REGION ((FLANK*2)+ALLELE) // 60
...
while (lineFileRow(lf, row))
    {
    seq5len=seq3len=allele1len=allele2len=obslen=altlen=0;
    counter[0]++; 
    for ( i = 0; i <= FLANK; i++) 
	{
	strcpy(seq5+i,"\0");
    ...
    if ( si == NULL ) counter[1]++; // rsId not found in database
    else
      {
	...
	if (!strncmp(obs  +allele1len, si->region+FLANK-seq5len+1, obslen-allele1len))
	  i += 1000; // observed
	if (!strncmp(alt  +allele2len, si->region+FLANK-seq5len+1, altlen-allele2len))
	  i += 100;  // alternate
	if (!strncmp(obsrc+allele1len, si->region+FLANK-seq3len+1, obslen-allele1len))
	  i += 10;   // observed RC
	if (!strncmp(altrc+allele2len, si->region+FLANK-seq3len+1, altlen-allele2len))
	  i += 1;    // alternate RC
	
	if ( seq5len + seq3len != 2*FLANK ) counter[2]++; // short flanking sequence
	if ( i == 0) counter[3]++; // database sequence does not match file sequence
	...
	if (i==1000)
	  ...
	  {
	    strncpy(dbSnp->observed,si->region,41);
	    strncpy(dbSnp->alternate,si->region,obslen);
	    //	    fprintf(f,"\t\t\t\t\t\t\t\t\t                    *\t\t\t\t                    *\n");
	    //	    fprintf(f,"\t\t\t\t\t\t\t\t\t%s\t%s\n",obs,obsrc);
	    //	    fprintf(f,"\t\t\t\t\t\t\t\t\t%s\t%s\n",alt,altrc);
	  }
	//    strncpy( &alternate, "-\0",2);// ????????? bug ??????

	//    dbSnp->allele1, allele1len, dbSnp->allele2, allele2len, si->region, &alternate);
	//	fprintf(f,"%i\t%s\t%s\t%s:%i\t%s:%i\t%s\t%s\n",
	//		si->region, &alternate);

	//    fprintf(f,"%i\t%s\t%s\t%s\t%s\t%s\t%s\n",
	//	    dbSnp->rsId, seq5, seq3, dbSnp->allele1, dbSnp->allele2,
