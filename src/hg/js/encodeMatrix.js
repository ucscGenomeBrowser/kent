...
var encodeMatrix = (function () {
    ...
    return {
        ...
        disableHelp: function (isOff) {
            ...
            if (isOff) {
                $.cookie(COOKIE_HELP, 'off', {expires: 90});
                $('.helpText').hide();
                $('.helpLauncher').show();
            } else {
                $.cookie(COOKIE_HELP, null);
                $('.helpText').show();
                $('.helpLauncher').hide();
            }
        },

       isHelpWanted: function () {
            // True if no cookie set
            return (!$.cookie(COOKIE_HELP));
        },
        ...
        start: function ($el) {
            ...
            // show only spinner and (optionally help) until data is retrieved
            $el.hide();
            encodeMatrix.disableHelp(!encodeMatrix.isHelpWanted());
            spinner = showLoadingImage(spinnerLoc, true);

            // setup click handlers for help controls
            $('.xIcon').click(function() {
                encodeMatrix.disableHelp(true);
            });
            $('.helpLauncher').click(function() {
                encodeMatrix.disableHelp(false);
            });
