...
if ( "$wc" == "0" ) then
 echo "errs found:"
 echo "$res2"
 echo "$res2" | mail -s "'Errors in hgTablesTestRobot on $HOST'" $USER hartera galt heather kuhn kent ann
 exit 1
endif
#
echo Done.
echo "$res2" | mail -s "'v${BRANCHNN} hgTablesTest robot done successfully on $HOST.'" $USER hartera galt heather kuhn kent ann
exit 0
