/* cdwQaEvaluate - Consider available evidence and set cdwValidFile.*QaStatus. */
 ...
 *  5 - Relaxing thresholds for CIRM2 experiments to be half of those for CIRM3. 
...
int failQa(struct sqlConnection *conn, struct cdwFile *ef, char *whyFormat, ...)
#ifdef __GNUC__
...
sqlSafef(query, sizeof(query), 
    "select count(*) from cdwQaFail where fileId=%u and qaVersion=%d", ef->id, version);
int prevFailCount = sqlQuickNum(conn, query);

/* If we are the first need to save it to database. */
if (prevFailCount == 0)
    {
    struct cdwQaFail fail = {.id=0, .fileId=ef->id, .qaVersion=version, .reason=reason};
    cdwQaFailSaveToDb(conn, &fail, "cdwQaFail", 0);
    }
return -1;
}

struct cdwQaRepeat *cdwQaRepeatMatching(struct sqlConnection *conn, 
	long long fileId, char *repClass)
/* Return record associated with repeat of given class and fileId */
{
char query[256];
sqlSafef(query, sizeof(query),
    "select * from cdwQaRepeat where fileId=%lld and repeatClass='%s'"
    , fileId, repClass);
return cdwQaRepeatLoadByQuery(conn, query);
}
...
double maxContam(struct sqlConnection *conn, struct cdwValidFile *vf, boolean isClose)
/* Get list of contamination targets that are close or far */
{
char query[512];
sqlSafef(query, sizeof(query), "select taxon from cdwAssembly where ucscDb='%s'", vf->ucscDb);
...
if (!isMammal)
    ...
    if (isClose)
        ...
        {
	sqlSafef(query, sizeof(query), "select max(mapRatio) from cdwQaContam where fileId=%u",
	    vf->fileId);
	}
    }
else
    {
    sqlSafef(query, sizeof(query), 
	"select max(mapRatio) from cdwQaContam c,cdwQaContamTarget t, cdwAssembly a "
...
boolean passContamination(struct sqlConnection *conn,
    struct cdwFile *ef, struct cdwValidFile *vf, struct cdwExperiment *exp,
...
boolean cdwBestPairedSomething(struct sqlConnection *conn, char *query, double *retBest)
...
boolean cdwBestCrossEnrichment(struct sqlConnection *conn, long long fileId, double *retBest)
...
char query[256];
sqlSafef(query, sizeof(query),
    "select max(sampleSampleEnrichment) from cdwQaPairSampleOverlap p,cdwValidFile e,cdwValidFile y "
    ...
    , fileId, fileId);
return cdwBestPairedSomething(conn, query, retBest);
}

boolean cdwBestPearsonClipped(struct sqlConnection *conn, long long fileId, double *retBest)
/* If there are biological replicates for same experiment and output type,  return
 * TRUE and calculate the best cross enrichment. */
{
char query[256];
sqlSafef(query, sizeof(query),
    "select max(pearsonClipped) from cdwQaPairCorrelation p,cdwValidFile e,cdwValidFile y "
    "where elderFileId=e.fileId and youngerFileId=y.fileId and e.replicate != y.replicate "
    "and (elderFileId = %lld or youngerFileId = %lld) "
    , fileId, fileId);
return cdwBestPairedSomething(conn, query, retBest);
...
if (sameString(vf->format, "fastq"))
    {
    /* Check a bunch of things either in cdwValidFile record or cdwFastqFile record. */
    struct cdwFastqFile *fq = cdwFastqFileFromFileId(conn, ef->id);
    if (fq == NULL)
       errAbort("No cdwFastqFile for file ID %u", ef->id);
    ...
    else if (!sameWord(fq->qualType, "sanger") && !solexaFastqOk)
        {
	...
	if (otherVf == NULL)
	    ...
	    {
	    struct cdwQaPairedEndFastq *pair = cdwQaPairedEndFastqFromVfs(conn, vf, otherVf,
			    NULL, NULL);
	    if (pair == NULL)
		errAbort("cdwQaPairedEndFastq not found for %u %u", vf->fileId, otherVf->fileId);
    ...
    if (passStat == 1)
        {
	struct cdwQaRepeat *rep = cdwQaRepeatMatching(conn, ef->id, "total");
	if (rep == NULL)
    ...
    if (passStat == 1)
        {
	struct cdwQaRepeat *rep = cdwQaRepeatMatching(conn, ef->id, "rRNA");
...
if (sameString(vf->format, "bam") || sameString(vf->format, "fastq") 
   ...
   || sameString(vf->format, "bigWig"))
	...
	{
	sqlSafef(query, sizeof(query),
	   "select e.enrichment from cdwQaEnrich e,cdwQaEnrichTarget t "
	...
	if (sameString(vf->format, "bigWig"))
	    {
	    double r = 0;
	    if (cdwBestPearsonClipped(conn, ef->id, &r))
		{
	    ...
	    if (cdwBestCrossEnrichment(conn, ef->id, &crossEnrichment))
		{
...
sqlSafef(query, sizeof(query),
    "update cdwValidFile set singleQaStatus=%d, replicateQaStatus=%d, qaVersion=%d "
    "where id=%u",  passStat, pairPassStat, version, vf->id);
sqlUpdate(conn, query);
}

void cdwQaEvaluate(int startFileId, int endFileId)
/* cdwQaEvaluate - Consider available evidence and set cdwValidFile.*QaStatus. */
{
...
for (ef = efList; ef != NULL; ef = ef->next)
	...
	if (!isEmpty(vf->experiment))
	    {
	    struct cdwExperiment *exp = cdwExperimentFromAccession(conn, vf->experiment);
	    if (exp != NULL)
		{
		char *dataType = exp->dataType;
...
cdwQaEvaluate(sqlUnsigned(argv[1]), sqlUnsigned(argv[2]));
return 0;
}
