...
static char const rcsid[] = "$Id: affyVsIlluminaCommon.c,v 1.3 2007/08/27 21:59:17 kent Exp $";
...
struct sqlResult *sr = sqlGetResult(conn, "select rsId from snpArrayAffy250Nsp");
char **row;
int affyCount = 0;
...
sr = sqlGetResult(conn, "select name from snpArrayIllumina300");
int illuminaCount = 0;
int bothCount = 0;
while ((row = sqlNextRow(sr)) != NULL)
    {
