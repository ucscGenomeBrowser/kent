...
#                   contents
#   requires external script: /root/browserLogs/scripts/trashSizeMonitor.sh
#
...
if [ -z "${SCRIPTS+x}" ]; then
  ${ECHO} "measureTrash.sh: ERROR: missing definition for SCRIPTS" 1>&2
  exit 255
fi
...
export atimeFile=`${SCRIPTS}/trashSizeMonitor.sh`
