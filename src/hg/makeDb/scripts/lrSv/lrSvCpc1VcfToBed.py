...
def emit(site, fout):
    ...
    # For DEL, POS is the non-deleted anchor base; drop it from the left (end
    # already = pos-1+ref_len). The shared prefix is >=1 base so this never
    # over-shifts. Only DEL moves; INS/MIXED keep the anchor.
    if sv_type == "DEL":
        start += 1
