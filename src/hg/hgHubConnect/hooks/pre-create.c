...
if (!(reqId && reqOffsetEnv && reqSizeEnv))
    ...
    if (errCatchStart(errCatch))
        ...
        char *reqServerName = jsonQueryString(req, "", "Event.Upload.MetaData.serverName", NULL);
        ...
        if (exitStatus == 0)
            {
            // set the location of the upload to the location it will ultimately live, which
            // depends on the machine name (euro, rr, beta, etc) and the user name
            char *location = setUploadPath(userName, reqFileName, reqParentDir, reqServerName, isHubToolsUpload);
            if (!location)
                {
