...
char *hubSpaceCommaSepFieldNames = "userName,fileName,fileSize,fileType,creationTime,lastModified,db,location,md5sum,parentDir,hubType";
...
char *hubType = el->hubType ? el->hubType : "trackHub";
// Exclude hubType from the UPDATE clause so a re-upload does not revert a
// row's hubType back to trackHub after the hub was promoted to assemblyHub
// (e.g. by a 2bit arriving later). The initial INSERT value stands.
sqlDyStringPrintf(update, "insert into %s values ( '%s','%s',%lld,'%s',NULL,'%s','%s','%s','%s','%s','%s') "
	"ON DUPLICATE KEY UPDATE fileSize=%lld, lastModified='%s', md5sum='%s', location='%s', db='%s'",
	tableName, el->userName, el->fileName, el->fileSize, el->fileType, el->lastModified, el->db, el->location, el->md5sum, el->parentDir, hubType,
...
ret->hubType = cloneString(row[10]);
return ret;
}

struct hubSpace *hubSpaceLoadAll(char *fileName)
/* Load all hubSpace from a whitespace-separated file.
 * Dispose of this with hubSpaceFreeList(). */
{
struct hubSpace *list = NULL, *el;
...
/* Load all hubSpace from a chopper separated file.
 * Dispose of this with hubSpaceFreeList(). */
{
struct hubSpace *list = NULL, *el;
struct lineFile *lf = lineFileOpen(fileName, TRUE);
...
ret->fileSize = sqlLongLongComma(&s);
...
if (sep == ',') fputc('"',f);
fputc(sep,f);
fprintf(f, "%lld", el->fileSize);
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->fileType);
if (sep == ',') fputc('"',f);
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->creationTime);
if (sep == ',') fputc('"',f);
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->lastModified);
if (sep == ',') fputc('"',f);
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->db);
...
fputc(sep,f);
if (sep == ',') fputc('"',f);
fprintf(f, "%s", el->hubType ? el->hubType : "trackHub");
if (sep == ',') fputc('"',f);
fputc(lastSep,f);
}
