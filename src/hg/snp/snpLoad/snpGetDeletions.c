/* snpGetDeletions -- generate a filtered set of deletions. */
...
/* not checking for position < chromSize; seqWithDeletions does this */
...
{
errAbort(
    "snpGetDeletions - generate file of filtered deletions\n"
    "usage:\n"
    "    snpGetDeletions database snpTable exceptionsTable\n");
}
...
while ((row = sqlNextRow(sr)) != NULL)
    ...
    if (sameString(exception, "MultipleAlignments") || sameString(exception, "ObservedWrongSize")) 
        {
...
void getDeletions()
/* assume snpTable already filtered on class = 'deletions' */
{
char query[512];
struct sqlConnection *conn = hAllocConn();
struct sqlResult *sr;
