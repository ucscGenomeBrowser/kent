...
static char const rcsid[] = "$Id: testBb.c,v 1.1 2009/02/01 01:45:51 kent Exp $";
...
int ppBedCmp(const void *va, const void *vb)
...
struct ppBed *ppBedLoadAll(char *fileName, struct hash *chromHash, struct lm *lm, 
	bits64 *retDiskSize)
...
double ppBedAverageSize(struct ppBed *pbList)
...
void makeChromInfo(struct ppBed *pbList, struct hash *chromSizeHash,
...
struct bwgSummary *summaryOnDepth(struct ppBed *pbList, struct bigWigChromInfo *chromInfoArray, 
	int reduction)
