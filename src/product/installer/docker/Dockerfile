...
RUN apt-get install -yq wget rsync && sed -i 's/101/0/g' /usr/sbin/policy-rc.d && chmod a+x /root/browserSetup.sh && /root/browserSetup.sh -b install && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
RUN chmod a+x /etc/service/mysqld/run /etc/service/apache/run
