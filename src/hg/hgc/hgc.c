...
static char const rcsid[] = "$Id: hgc.c,v 1.1429 2008/06/12 23:12:39 braney Exp $";
...
if (target != NULL)
	...
	{
	struct sqlConnection *conn = hAllocConn();
	seq = hDnaSeqGet(conn, psl->tName, target->seqTable,
			 target->extFileTable);
	hFreeConn(&conn);
    ...
    seq = hChromSeq(psl->tName, psl->tStart, psl->tEnd);
char *dna = seq->dna;
tolowers(dna);
if (psl->strand[0] == '-')
