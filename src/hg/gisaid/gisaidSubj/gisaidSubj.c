...
/* Table View */
hPrintf("<TD ALIGN=CENTER><A HREF=\"../cgi-bin/gisaidTable\" class='topbar' style='color:#FFFFFF;'>Table View</A></TD>");

/* Help */
hPrintf("<TD ALIGN=CENTER><A HREF=\"/goldenPath/help/gisaidTutorial.html#SubjectView\" target=_blank class='topbar' style='color:#FFFFFF;'>Help</A></TD>");
...
if (sameWord(curSubjId, ""))
    ...
    {
    safef(query, sizeof(query), "select subjId from %s.gisaidSubjInfo where subjId = '%s'",
    	  database, curSubjId);
...
cartHtmlStart("GISAID Subject View");
webMain(conn);
