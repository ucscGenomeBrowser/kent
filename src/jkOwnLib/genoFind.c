...
for (i = 0; i < gf->sourceCount; i++)
    ...
    mustWrite(f, &ss->start, sizeof(ss->start));
    mustWrite(f, &ss->end, sizeof(ss->end));
...
if (gf->segSize == 0)
    ...
    mustWrite(f, gf->allocated, count*sizeof(gf->listSizes[0]));
    }
else
    {
    // use endLists
    ...
    for (i = 0; i < gf->tileSpaceSize; i++)
        count += gf->listSizes[i];
    mustWrite(f, gf->allocated, count * sizeof(struct endList));
    }
}
...
mustWrite(f, &isTrans, sizeof(isTrans));
...
for (i = 0; i < gf->sourceCount; i++)
    ...
    if (fileNameLen != 0)
        {
        ss->fileName = needMem(fileNameLen);
        mustRead(f, ss->fileName, fileNameLen);
        }
    mustRead(f, &ss->start, sizeof(ss->start));
    mustRead(f, &ss->end, sizeof(ss->end));
...
static char* isTransDesc(boolean isTrans)
/* string description of index */
{
return isTrans ? "translated" : "untranslated";
}
...
if (strcmp(fileMagic, indexMagic))
    errAbort("wrong magic string for index file");
boolean wasTrans;
mustRead(f, &wasTrans, sizeof(wasTrans));
if (wasTrans != isTrans)
    errAbort("index file is %s, requesting an index for %s", isTransDesc(wasTrans), isTransDesc(isTrans));
