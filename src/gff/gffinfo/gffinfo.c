...
/* Make sure there's enough words in a line we process. */
...
for (;;)
    ...
    isNewGene = FALSE;
    if (wordCount >= 9 && strcmp(seg.group, curGroup) != 0)
	{
	...
	intron->start = seg.start;
	intron->end = seg.end;
	intron->frame = atoi(seg.frame);
	gffSegmentInsertSort(&gene->introns, intron);
	}
    else if (strcmp(seg.feature, "IG") == 0)	/* Safe to ignore. */
	{
	}
    else
	{
...
if (argc == 1)
    {
    printf("This program prints genes in a .gff file.\n");
