export db=hg38
export GENCODE_VERSION=V50
export PREV_GENCODE_VERSION=V49
...
hgsql knownGene${GENCODE_VERSION} -Ne "select geneSymbol, kgId from kgXref" | sort > ids.txt
join -t $'\t' genes.lst  ids.txt | tawk '{print $2,$1}' | sort > knownToMyGene2.txt
hgLoadSqlTab knownGene${GENCODE_VERSION} knownToMyGene2 ~/kent/src/hg/lib/knownTo.sql knownToMyGene2.txt
