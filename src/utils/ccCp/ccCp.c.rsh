...
for (i=start; i<hostCount; ++i)
	...
	{
	char ok = TRUE;
	if (rcpFile(fileName, newHost, fileName) != 0)
	    ok = FALSE;
	write(lockFd, &ok, 1);
	close(lockFd);
	if (ok )
		...
		rshSelf(hostList, newHost, i+1, fileName, lockDir);
	    }
	}
    }
}
...
if (childPid == 0)
	...
	printf("Copying %d of %d, errs in %d\n",
	    endCount, hostCount, errCount);
	if (finCount >= hostCount)
	    {
	    if (errCount > 0)
		{
		fprintf(stderr, "Errors copy to hosts:");
		for (i=0; i<hostCount; ++i)
		    {
		    char *host = hosts[i];
		    char *ln = lockName(lockDir, host);
		    lockFd = open(ln, O_RDONLY);
		    if (lockFd >= 0)
			{
			char ok = FALSE;
