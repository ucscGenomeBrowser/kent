...
if ( $status ) then
 ...
 echo "v$BRANCHNN build on beta failed." | mail -s "'v$BRANCHNN Build failed on beta'" $BUILDPERSON hartera galt heather kuhn ann kayla rhead archanat
 exit 1
endif
echo "build on beta done for v$BRANCHNN"
echo "v$BRANCHNN built successfully on beta." | mail -s "'v$BRANCHNN Build complete on beta'" $BUILDPERSON hartera galt heather kuhn kent ann kayla rhead archanat
