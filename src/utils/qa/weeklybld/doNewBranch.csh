...
if ( -e GitReports.ok ) then
    ...
    echo "Ready for pairings, day 16, Git reports completed for v${BRANCHNN} branch http://genecats.cse.ucsc.edu/git-reports/ (history at http://genecats.cse.ucsc.edu/git-reports-history/)." | mail -s "Ready for pairings (day 16, v${BRANCHNN} review)." $USER ${BUILDMEISTER} donnak@soe.ucsc.edu kuhn@soe.ucsc.edu ann@soe.ucsc.edu kate@soe.ucsc.edu luvina@soe.ucsc.edu steve@soe.ucsc.edu
    ...
    echo "Expected victims:\n${victims}" | mail -s "Code summaries for v$BRANCHNN are expected from...." $USER ${BUILDMEISTER} ann@soe.ucsc.edu donnak@soe.ucsc.edu
    foreach victim ( $victims )
		...
		if ($? == 0) then
		       rm -f /dev/shm/build.email.${victim}.txt
                       ...
                       echo "From: donnak@soe.ucsc.edu" >> /dev/shm/build.email.${victim}.txt
                       echo "Subject: Code summaries are due for ${victim}" >> /dev/shm/build.email.${victim}.txt
                       echo "Cc: donnak@soe.ucsc.edu" >> /dev/shm/build.email.${victim}.txt
