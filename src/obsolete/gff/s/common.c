...
/* fill a specified area of memory with char */
void fillBytes(vpt, count, fillChar)
void *vpt;
int count;
char fillChar;
{
...
if ((pt = calloc(1, size)) == NULL)
    {
    fprintf(stderr, "Out of memory asking for %s bytes\n",
	size);
    exit(-666);
    }
return pt;
