...
/* Allocate an array and fill it with dna from gene. */
void exonDna(gff,gene,dnaOut,dnaSizeOut)
struct gff *gff;
struct gffGene *gene;
...
void printProtein(gff, gene, out)
struct gff *gff;
struct gffGene *gene;
FILE *out;
{
...
if (dna != NULL)
    ...
    /* Someday I may have to figure out how
     * to incorperate the frame here. Works
     * as is for the Sanger C. elegans data set. */
    for (i=0; i<dnaSize-3; i+=3)
	{
	aa = lookupCodon(dna+i);
	if (!isupper(aa)) uglyf("<weird %d %c>", aa, aa);
	if (i == 0)
	    {
	    if (aa != 'M')
		{
		fprintf(stderr, "%s doesn't start with an M in %s\n",
...
if (argc != 3)
    {
    printf("This program prints the protein sequence for a gene\n");
...
if (gffOpen(&gff, gffName))
	...
	if (gffReadGenes(&gff))
	    ...
	    fprintf(out, ">%s putative protein sequence from %s\n",
		gene->name, gff.dnaName);
	    printProtein(&gff, gene, out);
	    fprintf(out, "\n");
	    }
