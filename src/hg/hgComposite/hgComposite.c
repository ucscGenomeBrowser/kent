/* hgComposite - bootstrapper / back end for the Data Integrator user interface
 ...
 * The UI view top level is in ../js/react/hgComposite/hgComposite.jsx
 * The UI model top level is in ../js/model/hgComposite/hgCompositeModel.js
 */
...
if (outFileOptions)
    ...
    if (doFile)
        {
        fileName = jsonOptionalStringField(outFileOptions, "fileName", "hgCompositeResults");
...
jsIncludeReactLibs();
jsIncludeFile("reactHgComposite.js", NULL);
jsIncludeFile("hgCompositeModel.js", NULL);

// Invisible form for submitting a query
static char *_hgCompositeName = "../cgi-bin/hgComposite"; /* Path to annotation intgrator */

printf("\n<form action=\"%s\" method=%s id='queryForm'>\n",
       _hgCompositeName, cartUsualString(cart, "formMethod", "POST"));
