...
static char const rcsid[] = "$Id: tableStatus.c,v 1.1 2005/11/04 17:29:51 kent Exp $";
...
ret->dataLength = sqlUnsigned(row[5]);
ret->maxDataLength = sqlUnsigned(row[6]);
ret->indexLength = sqlUnsigned(row[7]);
ret->dataFree = sqlUnsigned(row[8]);
ret->autoIncrement = sqlUnsigned(row[9]);
...
ret->dataLength = sqlUnsignedComma(&s);
ret->maxDataLength = sqlUnsignedComma(&s);
ret->indexLength = sqlUnsignedComma(&s);
ret->dataFree = sqlUnsignedComma(&s);
ret->autoIncrement = sqlUnsignedComma(&s);
...
fputc(sep,f);
fprintf(f, "%u", el->dataLength);
fputc(sep,f);
fprintf(f, "%u", el->maxDataLength);
fputc(sep,f);
fprintf(f, "%u", el->indexLength);
fputc(sep,f);
fprintf(f, "%u", el->dataFree);
fputc(sep,f);
fprintf(f, "%u", el->autoIncrement);
fputc(sep,f);
if (sep == ',') fputc('"',f);
