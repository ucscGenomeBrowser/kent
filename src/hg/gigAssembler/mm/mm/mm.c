...
/* A barge - a set of welded together clones. */
    ...
    struct dlList *belList;	/* List of barge elements. Order not significant. */
...
int oogCloneCmpSize(const void *va, const void *vb)
...
void mmAddMapInfo(char *fileName, struct mmClone *cloneList, struct hash *cloneHash)
...
{
fprintf(f, "%-12s %c %c     %-12s %c %c   %6d %6d  %d %d\n",  
	ocp->a->name, ocp->aHitS, ocp->aHitT, 
	ocp->b->name, ocp->bHitS, ocp->bHitT, 
	ocp->overlap, ocp->sloppyOverlap, ocp->externalPriority, ocp->score);
}
...
while (lineFileRow(lf, words))
    ...
    ocp->bHitS = words[6][0];
    ocp->bHitT = words[7][0];
    ocp->externalPriority = lineFileNeedNum(lf, words, 10);
...
struct dlList *clonesToBels(struct mmClone *cloneList)
/* Create a bargeEl for each clone and put it on doubly-linked list. */
{
struct mmClone *clone;
struct bargeEl *bel;
struct dlNode *node;
...
struct mmClone *cloneFromBelNode(struct dlNode *node)
/* Return clone associated with bel associated with dlNode. */
{
struct bargeEl *bel = node->val;
return bel->clone;
}

int nextBargeId()
/* Return unique ID for a new barge. */
{
static int id = 100;
return id++;
}
...
struct bargeEl *bel = clone->bel;

AllocVar(barge);
barge->id = nextBargeId();
barge->cloneEndList = newDlList();
barge->belList = newDlList();
dlAddValTail(barge->belList, bel);
...
dlAddValTail(barge->belList, clone->bel);
clone->barge = barge;
...
struct bargeEl *belA = cloneA->bel, *belB = cloneB->bel;
struct barge *barge;
int orientation;
boolean addAfter;
...
AllocVar(cc);
cc->belList = dlClone(orig->belList);
...
struct dlNode *node;
struct bargeEl *bel;
for (node = barge->belList->head; !dlEnd(node); node = node->next)
    {
    bel = node->val;
...
int aCount = dlCount(bargeA->belList), bCount = dlCount(bargeB->belList);
boolean ok;
...
if (!ok)
    ...
    freeDlList(&little->belList);
    freez(&little);
    }
freeDlListAndVals(&backup->cloneEndList);
freeDlList(&backup->belList);
freez(&backup);
return ok;
}
...
for (clone = cloneList; clone != NULL; clone = clone->next)
    {
    if ((outerBel = clone->bel->enclosed) != NULL)
	dlAddValTail(enclosedList, clone);
    }
dlSort(enclosedList, oogCloneCmpSize);
...
for (node = enclosedList->head; !dlEnd(node); node = node->next)
    ...
    outerBel = clone->bel->enclosed;
    while (outerBel->enclosed != NULL)
	outerBel = outerBel->enclosed;
...
for (ignoreEnds = 0; ignoreEnds <= 1; ++ignoreEnds)
	...
	if (ocp->score < 0)
	    break;
	ocpDump(ocp, logFile);
	if (ocp->a->bel->enclosed)
	    {
	    logIt("skipping enclosed: %s inside %s\n\n",
		ocp->a->name, ocp->a->bel->enclosed->clone->name);
	    continue;
	    }
	if (ocp->b->bel->enclosed)
	    {
	    logIt("skipping enclosed: %s inside %s\n\n",
		ocp->b->name, ocp->b->bel->enclosed->clone->name);
	    continue;
	    }
    #ifdef SOMETIMES
...
for (; !dlEnd(node); node = node->next)
    {
    ce = node->val;
    if (ce->isStart && ce->clone->bel->enclosed == NULL)
	 return ce->clone;
    }
return NULL;
}
...
boolean evalGapInfo(FILE *f, struct barge *aBarge, struct barge *bBarge, 
	struct hash *ocpHash, struct hash *cloneHash, struct hash *bepHash)
...
if (joiningBep == NULL)
	...
	for (bepRef = joiningBep; bepRef != NULL; bepRef = bepRef->next)
	    {
	    bep = bepRef->bep;
	    if (startsWith("RPCI", bep->cloneName))
		fprintf(f, " %s", bep->cloneName);
	    else
		fprintf(f, " %s&%s", bep->a, bep->b);
	    }
	fprintf(f, "\n");
	}
    bridged = TRUE;
...
void saveBargeFile(char *fileName, struct dlList *bargeList, struct hash *ocpHash,
...
fprintf(f, "Barge (Connected Clone) File mm Version %d\n\n", version);
fprintf(f, "start    name  phase  size     before         after         mapPos\n");
fprintf(f, "---------------------------------------------------------------------------\n");
...
for (bNode = bargeList->head; !dlEnd(bNode); bNode = bNode->next)
    ...
    barge = bNode->val;
    cloneCount = dlCount(barge->belList);
	...
	if (ce->isStart)
		...
		if (ocp->a == outer)
	        ...
	        fprintf(f, "     (buried %-9s %d %6d   %c %c  %9s %6d   %c %c %8d)\n",
		    clone->name, clone->phase, clone->size, os, ot, 
		...
		fprintf(f, "%-8d %-9s %d %6d\t", pos, clone->name, clone->phase, clone->size);
	...
	{
	boolean bridged;
	bridged = evalGapInfo(f, bNode->val, bNode->next->val, ocpHash, cloneHash, bepHash);
	if (bridged)
	    pos += 50000;
	else
	    pos += 100000;
...
void orderAndOrientBarges(struct dlList *bargeList)
...
mmAddMapInfo(fileName, cloneList, cloneHash);
...
orderAndOrientBarges(bargeList);
...
saveBargeFile(fileName, bargeList, ocpHash, cloneHash, bepHash);
}
