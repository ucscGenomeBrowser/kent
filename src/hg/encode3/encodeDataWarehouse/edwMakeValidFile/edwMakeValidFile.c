...
static void requireStartEndLines(char *fileName, char *reportFileName, char *startLine, char *endLine)
...
{
requireStartEndLines(path, ef->submitFileName, "<Header>", "</Messages>");
}
...
if (!fileStartsWithOneOfPair(path, "IDAT", "DITA"))
    errAbort("%s is not a valid .idat file, it does not start with IDAT or DITA", ef->submitFileName);
}
