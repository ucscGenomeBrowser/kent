...
if ( "$1" != "opensesame" ) then
    echo
    echo "Do not run standalone. This script should be called from doNewBranch.csh."
    echo
    exit 0
endif 

if ( "$HOST" != "$BOX32" ) then
 echo "Error: this script must be run from $BOX32."
 exit 1
endif
...
echo
echo  "NOW STARTING 32-BIT BUILD ON $HOST"
echo
...
./coBranch.csh
if ( $status ) then
    echo "checkout 32-bit branch sandbox on $HOST failed"
    exit 1
endif
#echo debug: disabled build 32-bit utils on $HOST
./buildUtils.csh
if ( $status ) then
    echo "build 32-bit utils on $HOST failed"
    exit 1
endif
...
#ssh $BOX32 $WEEKLYBLD/buildCgis.csh
if ( $status ) then
