...
ret->alignGapCount = sqlSigned(row[33]);
ret->processedIntrons = sqlSigned(row[34]);
ret->conservedSpliceSites = sqlSigned(row[35]);
ret->maxOverlap = sqlSigned(row[36]);
...
ret->alignGapCount = sqlSignedComma(&s);
