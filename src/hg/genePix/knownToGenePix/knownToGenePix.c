/* knownToGenePix - Create knownToGenePix table by riffling through various other knownTo tables. */
#include "common.h"
#include "linefile.h"
#include "hash.h"
#include "dystring.h"
...
void knownToGenePix(char *database)
/* knownToGenePix - Create knownToGenePix table by riffling through various other knownTo tables. */
{
char *tempDir = ".";
char *outTable = "knownToGenePix";
FILE *f = hgCreateTabFile(tempDir, outTable);
struct sqlConnection *hConn = sqlConnect(database);
struct sqlConnection *iConn = sqlConnect("genePix");
struct sqlResult *sr;
char query[512];
char **row;
